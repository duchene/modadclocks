...
getPPstats <- function(N = 200){
	...
	emplogstats <- rbind(apply(empost, 2, mean), apply(empost, 2, sd))
	...
	emplogstats <- cbind(emplogstats, treeLength = c(mean(emptrlens), sd(emptrlens)))
	...
	for(i in 1:length(ppsfiles)){
	      ...
	      ppsmlik[i] <- multlik(ppsal)
              ppslogstats[[i]] <- rbind(apply(ppspost, 2, mean), apply(ppspost, 2, sd))
			...
			print(mean(phylogs[[2]]$edge.length))
              ...
              ppslogstats[[i]] <- cbind(ppslogstats[[i]], treeLength = c(mean(ppstrlens), sd(ppstrlens)))
